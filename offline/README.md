...
完整使用手册请看 [../docs/user_manual.md](../docs/user_manual.md)，其中包含输入格式、模型选择、run-out 标识、输出文件和 Origin 排错。
...
output\e739_runout_data.csv  # 仅当存在 run-out 时生成
...
Origin 项目中包含总汇总表、每组有效点、线性化值、置信带采样点、重复水平统计和工程习惯的 S-N / ε-N 图。默认不生成 E739 线性化图；如需同时输出线性化图，可添加 `--linearized-graph`。
...
所有 E739 模型都会识别 `--status` 中的 run-out / suspended 行。标准模型和 `shifted-log` 不把 run-out 点作为失效点拟合，但会输出 `e739_runout_data.csv` 并在 Origin 图中用空心散点加右箭头标识；如需先隐藏箭头，可添加 `--no-runout-arrows`。如果需要三参数模型 `log10(N) = A + B log10(response - C)` 并把 run-out 作为右删失数据处理，运行时加：

```powershell
.\.venv\Scripts\python.exe -m originnsfit --input data --output output --pattern "*.csv" --status "状态" --e739-model threshold_log_mle
```

汇总表会额外输出 `coefficient_c`、`threshold`、`sigma`、`log_likelihood`、`negative_log_likelihood`、`n_failure`、`n_runout`、`success` 和优化器信息；Origin 图中的公式也会同步显示 `(响应量 - C)`。可运行 `.\.venv\Scripts\python.exe .\scripts\verify_threshold_log_mle.py` 做最小验证。
