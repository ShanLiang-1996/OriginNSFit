...
此时工程图公式会显示为 `N_f = a * (响应量 - C)^b`，汇总表会额外给出 `coefficient_c`、`threshold`、`sigma`、`log_likelihood`、`negative_log_likelihood`、`n_failure`、`n_runout`、`success` 和优化器信息。可运行 `.\.venv\Scripts\python.exe .\scripts\verify_threshold_log_mle.py` 做最小验证。
