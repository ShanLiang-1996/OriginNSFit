...
也可以用 `--e739-x-transform linear` 切换为 `X = 应力/应变响应`。如果需要带阈值和 run-out 右删失的三参数最大似然模型，可使用 `--e739-model threshold_log_mle`，对应 `log10(N) = A + B log10(response - C) + error`。程序会输出参数估计、置信区间、整条中位曲线的置信带、重复水平统计和线性充分性 F 检验；其中标准 E739 流程只适用于失效寿命数据，`threshold_log_mle` 会在指定 `--status` 时把 run-out / suspended 行作为右删失数据处理。
...
- `寿命`：疲劳寿命 N，必须为正数。
- `塑性应变幅` / `应变最大值` / `应变幅` / `应力幅` / `stress` / `strain`：受控响应量，默认按 `log10` 进入 E739 的 X。
...
完整使用流程、模型说明、run-out 标识和 Origin 排错请看 [docs/user_manual.md](docs/user_manual.md)。
...
output/e739_runout_data.csv         存在 run-out 时生成，记录 run-out 点及绘图坐标
...
Origin 项目中包含总汇总工作簿、每组数据的有效点表、置信带采样表、重复水平统计表，以及工程习惯的 S-N / ε-N 图。默认不生成 E739 线性化图；如需同时输出线性化图，可添加 `--linearized-graph`。
...
如果旧版 Origin 对 `.otpu` 模板不兼容，可以加 `--no-graph-template`。E739 图形绘制默认走 LabTalk `plotxy` 兼容路径，适配旧版 Origin 时会把散点、拟合线和置信带明确追加到同一个图层，避免每条曲线单独新建图。Origin 自动化失败时会写出 `output/origin_automation.log`，方便在离线电脑上查看真实错误。

工程图公式会显示为 `N_f = a * (响应量)^b`，其中 `a = 10^A`、`b = B`，由 E739 的 `log10(N) = A + B log10(response)` 换算得到。响应列名可以是 `塑性应变幅`、`应变最大值`、`应变幅`、`应力幅`、`strain`、`stress` 等；程序只改变图中文字和坐标名称，拟合原理保持一致。
...
所有模型都会识别 `--status` 中的 run-out / suspended 行。标准 E739 和 shifted-log 不把 run-out 作为失效点拟合，但会导出 `e739_runout_data.csv` 并在 Origin 图中用空心散点加右箭头表示；如需先隐藏箭头，可添加 `--no-runout-arrows`。`threshold_log_mle` 会把 run-out 作为右删失数据进入似然。此时工程图公式会显示为 `N_f = a * (响应量 - C)^b`，汇总表会额外给出 `coefficient_c`、`threshold`、`sigma`、`log_likelihood`、`negative_log_likelihood`、`n_failure`、`n_runout`、`success` 和优化器信息。可运行 `.\.venv\Scripts\python.exe .\scripts\verify_threshold_log_mle.py` 做最小验证。
