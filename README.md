...
也可以用 `--e739-x-transform linear` 切换为 `X = 应力/应变响应`。如果需要三参数模型，可使用 `--e739-model shifted-log`，对应 `log10(N) = A + B log10(response - C)`，待拟合参数为 A、B、C。程序会输出参数估计、置信区间、整条中位曲线的置信带、重复水平统计和线性充分性 F 检验。E739 的这些基础流程只适用于失效寿命数据；如果数据中有 run-out / suspended test，请用 `--status` 指定状态列，程序会在结果中给出警告。
...
```powershell
.\.venv\Scripts\python.exe -m originnsfit --input data --output output --pattern "*.csv" --e739-model shifted-log
```

此时工程图公式会显示为 `N_f = a * (响应量 - C)^b`，汇总表会额外给出 `coefficient_c`、`standard_error_c` 和 C 的置信区间。
