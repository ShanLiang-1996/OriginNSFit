...
其中 `life` 默认识别为 `寿命` 列，`response` 默认识别为 `应变幅` 或 `应力幅` 列。横坐标在 Origin 图中使用 `log10` 对数坐标，纵坐标保持线性坐标。
...
离线调整 Origin 绘图代码时，可参考 [docs/originpro_plotting_manual.md](docs/originpro_plotting_manual.md)。
...
output/fit_lines.csv      log-log 空间中的直线斜率、截距和端点，用于检查幂律拟合
```
